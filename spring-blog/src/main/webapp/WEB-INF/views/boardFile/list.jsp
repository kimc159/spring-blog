...
<div class="boardArea">
	 <div class="searchArea">
		 <form action="/boardFile/list" method="post">
		 	...
		 	<select name="searchType">
		 		...
		 		<option value="title" <c:if test="${scri.searchType = 'all'}">selected</c:if>>제목</option>
		 		<option value="content" <c:if test="${scri.searchType = 'all'}">selected</c:if>>내용</option>
		 	</select> 
			...
			<tbody>
				<c:forEach items="${list}" var="list"> 
					<tr> 
						...
						<a href="/boardFile/boardDetail?seq=${list.seq}&writer=${list.writer}"> ${list.title}</a>
						</td>  
...
$(function() {
	$("#perPageSelect").on("change", function() {
		...
		location.href="/boardFile/list?page=1&perPageNum=" + perPageNum + "&searchType=${scri.searchType}&keyword=${scri.keyword}";
	});
});
</script>
</body>
